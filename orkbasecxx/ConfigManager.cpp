	...
	try
	{
		...
		if(!cfgFilename) {
			...
			else
			{
                ...
                CStdString logMsg;
                logMsg.Format("xxxxx  config File Name %s", (char*)(cfgFilename));
                LOG4CXX_INFO(LOG.configLog,  logMsg);
		...
		doc = m_parser->getDocument();
                ...
                LOG4CXX_INFO(LOG.configLog, " doc xxxx");
		...
		if (doc)
		{
                        LOG4CXX_INFO(LOG.configLog, "get First Child");
			DOMNode *firstChild = doc->getFirstChild();
			if (firstChild)
			{
                                LOG4CXX_INFO(LOG.configLog, "Serialize Dom");
