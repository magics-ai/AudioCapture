...
//////////////////////////////////////////////////////////////////////////
// Pack for MDF_READFILE|MDF_WRITEFILE
	if ((m_nDocFlags & CMarkup::MDF_READFILE) && (m_nDocFlags & CMarkup::MDF_WRITEFILE))
	{
...
bool FilePos::FileClose()
{
	if ( m_fp )
	{
		...
		m_fp = NULL;
//////////////////////////////////////////////////////////////////////////
		m_nDocFlags &= ~(CMarkup::MDF_WRITEFILE|CMarkup::MDF_READFILE|CMarkup::MDF_APPENDFILE); //Pack 
//////////////////////////////////////////////////////////////////////////
		return true;
	}
	return false;
...
//////////////////////////////////////////////////////////////////////////
bool CMarkup::Open( MCD_CSTR szFileName, int nDocFlags, MCD_STR* pstrEncoding)
{

	m_bFileMode = true;
	m_nFileFlags = nDocFlags;
	m_strFileName = szFileName;
	if ( ! ReadTextFile(m_strFileName = szFileName, m_strDoc, &m_strResult, &m_nDocFlags) )
		return false;
	return x_ParseDoc();
}
bool CMarkup::Close()
...
int CMarkup::GetElemIndex() const
{
	return m_iPos;
}
void CMarkup::GotoElemIndex( int nIndex )
{
	m_iPos = nIndex;
	m_iPosParent = ELEM(m_iPos).iElemParent;
	m_iPosChild = 0;
}
int CMarkup::GetDocElemCount() const
{
	return m_pElemPosTree->GetSize() ;
}
...
bool CMarkup::FindPrevElem( MCD_CSTR szName )
{
	if (szName == NULL)
	{
		if (ELEM(m_iPos).iElemPrev)
		{
			x_SetPos(ELEM(ELEM(m_iPos).iElemPrev).iElemParent,ELEM(m_iPos).iElemPrev,0);
			return true;
		}else{
			return false;
		}
...
// 		int nCount = 0;
// 		MCD_STR strTagName = x_GetTagName( m_iPos );
// 		PathPos path( MCD_2PCSZ(strTagName), false );
// 		int iPosParent = ELEM(iPos).iElemParent;
// 		int iPosSib = 0;
// 		while ( iPosSib != m_iPos )
// 		{
// 			path.RevertOffset();
...
bool CMarkup::FindPrevChildElem( MCD_CSTR szName)
{
	if (szName == NULL)
	{
		...
		if (m_iPosChild && ELEM(m_iPosChild).iElemPrev )
		{
			x_SetPos( ELEM(m_iPos).iElemParent, m_iPos, ELEM(m_iPosChild).iElemPrev );
			return true;
		}else{
			return false;
		}		
...
bool CMarkup::HasAttrib( MCD_CSTR szAttrib ) const
{
...
bool CMarkup::HasChildAttrib( MCD_CSTR szAttrib ) const
{
	if ( m_nDocFlags & MDF_READFILE )
		return false;
	int nNodeStart = 0;
	int iPos = m_iPosChild;
	if ( iPos && m_nNodeType == MNT_ELEMENT )
		nNodeStart = ELEM(iPos).nStart;
	else if ( iPos == m_iPosChild && m_nNodeLength && m_nNodeType == MNT_PROCESSING_INSTRUCTION )
		nNodeStart = m_nNodeOffset;
	else
		return false;
...
bool CMarkup::FindSetData( MCD_CSTR szName, MCD_CSTR szData, int nFlags )
{
	if (FindElem(szName))
	{
		return SetData(szData,nFlags);
	}else{
		return false;
	}

}
MCD_STR CMarkup::EncodeBase64(
...
int CMarkup::DecodeBase64(
								 const MCD_STR& strBase64,
